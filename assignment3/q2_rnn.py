...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        padding = max_length - len(sentence)
        sentence = (sentence + [zero_vector]*padding)[:max_length]
        labels = (labels + [zero_label]*padding)[:max_length]
        mask = (mask + [False]*padding)[:max_length]
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.mask_placeholder = tf.placeholder(tf.int32, [None, self.max_length])
        self.dropout_placeholder = tf.placeholder(tf.float32)
        ### END YOUR CODE

    def create_feed_dict(self, inputs_batch, labels_batch=None, mask_batch=None, dropout=1):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable("U", [self.config.hidden_size, self.config.n_classes], dtype=tf.float32,
          initializer=tf.contrib.layers.xavier_initializer())
        b_2 = tf.get_variable("b2", [self.config.n_classes], dtype=tf.float32, initializer=tf.zeros_initializer)
        h_t = tf.zeros(name="h0", shape=[1, self.config.hidden_size], dtype=tf.float32)
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        cross_entropy = tf.boolean_mask(cross_entropy, self.mask_placeholder)

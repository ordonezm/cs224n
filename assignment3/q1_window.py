...
class Config:
    ...
    n_window_features = n_word_features * (1 + window_size*2)
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i, _ in enumerate(sentence):
            window = []
            window.extend([start for k in range(window_size-i)])
            window.extend(sentence[max(0,i-window_size):min(i+window_size+1,len(sentence))])
            window.extend([end for k in range(window_size-len(sentence)+i+1)])
            windowed_data.append(([component for word in window for component in word], labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        self.W1 = tf.get_variable("W1", [int(x.shape[1]), self.config.hidden_size], dtype=tf.float32,
          initializer=tf.contrib.layers.xavier_initializer())
        self.b1 = tf.get_variable("b1", [self.config.hidden_size], dtype=tf.float32, initializer=tf.zeros_initializer)
        h = tf.nn.relu(tf.matmul(x, self.W1) + self.b1)
        h_drop = tf.nn.dropout(h, dropout_rate)

        self.U = tf.get_variable("U", [self.config.hidden_size, self.config.n_classes], dtype=tf.float32,
          initializer=tf.contrib.layers.xavier_initializer())
        self.bu = tf.get_variable("bu", [self.config.n_classes], dtype=tf.float32, initializer=tf.zeros_initializer)

...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        # Use the model to predict the next transition for each partial parse
        batch = unfinished_parses[0:batch_size]
        transitions = model.predict(batch)
        for k in range(len(batch)):
            batch[k].parse_step(transitions[k])

        # remove completed parses
        for k in range(len(batch)-1,-1,-1):
            if not batch[k].buffer and len(batch[k].stack) == 1:
                unfinished_parses.pop(k)

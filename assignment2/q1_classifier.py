...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        # self.W = tf.Variable(tf.random_uniform((self.config.n_features, self.config.n_classes), minval=0, maxval = 0.01))
        ...
        scores = tf.matmul(self.input_placeholder, self.W) + self.b

        stability = tf.reduce_max(scores, axis=1, keepdims=True)
        scores = scores - stability

        exp_scores = tf.exp(scores)
        pred = exp_scores / tf.reduce_sum(exp_scores, axis=1, keepdims=True)
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        log_likelihood = self.labels_placeholder * tf.log(pred)
        loss = tf.negative(tf.reduce_sum(log_likelihood), name="cross_entropy_loss")
        ### END YOUR CODE
        return loss
